...
function renderGate(){ const ok=isLogged(); if(el('login-section')) el('login-section').style.display=ok?'none':'block'; if(el('dashboard')) el('dashboard').style.display=ok?'block':'none'; if(ok) loadData(); }
...
function ensureLots(n){ n.history=Array.isArray(n.history)?n.history:[]; n.lots=Array.isArray(n.lots)?n.lots:[]; if(typeof n.dividen!=='number') n.dividen=0;
  if(n.lots.length===0){ let lots=[]; const sorted=[...n.history].sort((a,b)=>(a.ts||0)-(b.ts||0));
    for(const h of sorted){ const a=Number(h.amount||0);
      if(h.type==='tambah'&&a>0) lots.push({ts:h.ts||Date.now(),amount:a,remaining:a});
      else if(h.type==='tarik'&&a>0){ let need=a; for(const l of lots){ if(need<=0)break; const take=Math.min(l.remaining,need); l.remaining-=take; need-=take; } }
      else if(h.type==='koreksi'){ if(a>0) lots.push({ts:h.ts||Date.now(),amount:a,remaining:a}); if(a<0){ let need=-a; for(const l of lots){ if(need<=0)break; const take=Math.min(l.remaining,need); l.remaining-=take; need-=take; } } }
    }
    n.lots=lots.filter(l=>l.remaining>0);
    if(n.lots.length===0 && Number(n.saldo||0)>0){ n.lots=[{ts:Date.now(),amount:Number(n.saldo||0),remaining:Number(n.saldo||0)}]; }
  } return n;
}
...
function adminFee(x){ const n=Number(x||0); if(n<=0) return 0;
  if(n<350000) return 3000; if(n<600000) return 5000; if(n<800000) return 7000;
  if(n<2000000) return 10000; if(n<5500000) return 20000; if(n<6500000) return 25000;
  if(n<15000000) return 30000; const extra=n-15000000; const blocks=Math.ceil(extra/1000000); return 30000+blocks*2000; }

/* ===== Donut Chart (data-square=1 untuk bulat) ===== */
...
/* ===== Admin Shell (tabel dibungkus table-wrap) ===== */
function adminShell(){
  return `
  <div class="cards">
...
/* ===== Bind admin inputs ===== */
function wireAdminInputs(){ ['saldoBaru','saldoEdit','divJumlah','revAmount'].forEach(id=>{ const x=el(id); if(!x) return; x.addEventListener('input', ()=> x.value=fmtNum(num(x.value))); }); }
function refreshNameLists(){ const list=(state.nasabah||[]).map(x=>({label:`${x.nama} — ${x.id}`, value:x.id})).sort((a,b)=>a.label.localeCompare(b.label,'id')); const fill=sid=>{const s=el(sid); if(!s) return; s.innerHTML=list.map(r=>`<option value="${r.value}">${r.label}</option>`).join('')}; ['namaEditSel','divNamaSel','riwNama'].forEach(fill); }
...
function renderAdmin(){
  ...
  root.innerHTML=adminShell(); wireAdminInputs();
  ...
  list.forEach(n=>{
    ...
    tr.innerHTML=`<td>${n.nama}</td><td>${n.id}</td><td>${fmt(n.saldo||0)}</td><td>${fmt(n.dividen||0)}</td>
      <td><a class="small" href="${link}" target="_blank" rel="noopener">Buka</a>
      <button class="small" data-copy="${link}">Salin</button>
      <button class="danger small" data-del="${n.id}">Hapus</button></td>`;
    body.appendChild(tr);
  });
  body.querySelectorAll('button[data-copy]').forEach(b=>{ b.addEventListener('click', async ()=>{ const url=b.getAttribute('data-copy'); try{ await navigator.clipboard.writeText(url); b.textContent='Disalin'; setTimeout(()=>b.textContent='Salin',1000);}catch{ prompt('Salin link ini:', url);} }); });
  body.querySelectorAll('button[data-del]').forEach(b=>{ b.addEventListener('click', async ()=>{ const id=b.getAttribute('data-del'); const n=findById(id); if(!n) return; if(!confirm(`Hapus nasabah "${n.nama}"?`)) return; state.nasabah=(state.nasabah||[]).filter(x=>x.id!==id); try{ await apiPut(state); renderAdmin(); }catch(e){ alert(e.message); } }); });

  refreshNameLists();

  const sorted=[...list].sort((a,b)=>Number(b.saldo||0)-Number(a.saldo||0));
  const top=sorted.slice(0,8); const others=sorted.slice(8).reduce((s,n)=>s+Number(n.saldo||0),0);
  const parts=top.map(n=>({label:n.nama.split(' ')[0], value:Number(n.saldo||0)})); if(others>0) parts.push({label:'Lainnya', value:others});
  lastAdminParts=parts.length?parts:[{label:'Kosong', value:1}];
  drawDonut(el('adminPie'),lastAdminParts,PALETTE); renderLegend(el('adminLegend'),lastAdminParts,PALETTE);

  // actions
  el('btnTambah').addEventListener('click', async ()=>{
    const nama=(el('namaBaru').value||'').trim(); const saldo=num(el('saldoBaru').value);
    if(!nama){ alert('Nama wajib'); return; }
    const exist=(state.nasabah||[]).some(x=>(x.nama||'').toLowerCase()===nama.toLowerCase()); if(exist){ alert('Nama sudah ada'); return; }
    const id=genId((state.nasabah||[]).map(x=>x.id)); const now=Date.now();
    const hist=saldo>0?[{ts:now,type:'tambah',amount:saldo,note:'Setoran awal'}]:[]; const lots=saldo>0?[{ts:now,amount:saldo,remaining:saldo}]:[];
    state.nasabah=[...(state.nasabah||[]),{id,nama,saldo,dividen:0,history:hist,lots}]; try{ await apiPut(state); el('namaBaru').value=''; el('saldoBaru').value=''; renderAdmin(); }catch(e){ alert(e.message); }
  });

  el('btnEdit').addEventListener('click', async ()=>{
    const id=el('namaEditSel').value; const jumlah=num(el('saldoEdit').value); const mode=el('aksiEdit').value;
    const note=(el('catatanEdit').value||'').trim(); const ts=buildTs(el('tglEdit').value, el('jamEdit').value);
    ...
    let delta=jumlah;
    if(mode==='kurangi'){ if(jumlah>Number(n.saldo||0)){ alert('Tarik melebihi saldo'); return; } delta=-Math.abs(jumlah); n.lots=lotsConsume(n.lots,Math.abs(delta)); }
    else if(mode==='tambah'){ n.lots=lotsAdd(n.lots,delta,ts); }
    else if(mode==='koreksi'){ delta=jumlah-Number(n.saldo||0); if(delta>=0) n.lots=lotsAdd(n.lots,delta,ts); else n.lots=lotsConsume(n.lots,Math.abs(delta)); }
    ...
    state.nasabah[idx]=n; try{ await apiPut(state); ['saldoEdit','catatanEdit','tglEdit','jamEdit'].forEach(i=>el(i).value=''); renderAdmin(); if(el('riwNama').value===id) renderHistory(id); }catch(e){ alert(e.message); }
  });

  el('divBtn').addEventListener('click', async ()=>{
    const id=el('divNamaSel').value; const jumlah=num(el('divJumlah').value); const mode=el('divAksi').value;
    const ts=buildTs(el('divTgl').value, el('divJam').value); const note=(el('divCatatan').value||'').trim()||(mode==='tambah'?'Penambahan dividen':'Penarikan dividen');
    if(!id||!jumlah){ alert('Pilih nasabah & isi nominal'); return; }
    const idx=indexById(id); if(idx<0){ alert('Nasabah tidak ditemukan'); return; }
    const n={...state.nasabah[idx]}; const delta=(mode==='tambah')?jumlah:-jumlah; if(mode==='kurangi'&&jumlah>Number(n.dividen||0)){ alert('Lebih besar dari dividen saat ini'); return; }
    n.dividen=Math.max(0,Number(n.dividen||0)+delta); n.history=[...(n.history||[]),{ts,type:'dividen',amount:Math.abs(jumlah),note}];
    state.nasabah[idx]=n; try{ await apiPut(state); ['divJumlah','divCatatan','divTgl','divJam'].forEach(i=>el(i).value=''); renderAdmin(); if(el('riwNama').value===id) renderHistory(id); }catch(e){ alert(e.message); }
  });

  el('riwRefresh').addEventListener('click', ()=> renderHistory(el('riwNama').value));
}

function buildTs(d,t){ if(!d&&!t) return Date.now(); const [Y,M,D]=(d||'').split('-').map(v=>parseInt(v,10)); const [h,m]=(t||'').split(':').map(v=>parseInt(v,10));
...
function renderHistory(id){
  const tb=el('riwTable').querySelector('tbody'); tb.innerHTML=''; const n=findById(id);
  if(!n){ el('riwEmpty').style.display='block'; return; }
  const list=Array.isArray(n.history)?n.history:[]; if(list.length===0){ el('riwEmpty').style.display='block'; return; } el('riwEmpty').style.display='none';
  const data=list.map((x,i)=>({...x,_i:i})).sort((a,b)=>(b.ts||0)-(a.ts||0));
  data.forEach((it,row)=>{ const d=new Date(it.ts||Date.now()); const tgl=d.toLocaleString('id-ID',{dateStyle:'medium',timeStyle:'short'}); const jenis=(it.type||'koreksi').toLowerCase();
    const tr=document.createElement('tr'); tr.innerHTML=`<td>${row+1}</td><td>${tgl}</td><td>${jenis[0].toUpperCase()+jenis.slice(1)}</td><td>${fmt(it.amount||0)}</td><td>${it.note||'-'}</td><td><button class="danger small" data-del="${it._i}" data-id="${n.id}">Hapus</button></td>`; tb.appendChild(tr); });
  tb.querySelectorAll('button[data-del]').forEach(b=>{ b.addEventListener('click', async ()=>{ const i=parseInt(b.getAttribute('data-del'),10); const id=b.getAttribute('data-id'); if(!confirm('Hapus catatan ini? (Tidak mengubah saldo/lots/dividen)')) return; const idx=indexById(id); if(idx<0) return; const copy={...state.nasabah[idx]}; copy.history=(copy.history||[]).filter((_,k)=>k!==i); state.nasabah[idx]=copy; try{ await apiPut(state); renderHistory(id); }catch(e){ alert(e.message); } }); });
}

/* ===== Load Data ===== */
async function loadData(){
  try{
    const data=await apiGet(); if(!Array.isArray(data.nasabah)) data.nasabah=[];
    let changed=false; const taken=[];
    state.nasabah=data.nasabah.map(x=>{ const n={...x}; if(!n.id){ n.id=genId(taken); changed=true; } taken.push(n.id); ensureLots(n); if(typeof n.dividen!=='number'){ n.dividen=0; changed=true; } return n; });
    if(changed){ try{ await apiPut(state); }catch{} }
    renderAdmin();
  }catch(e){ const root=el('admin-root'); if(root) root.innerHTML=`<p class="muted">Gagal memuat data: ${e.message||e}</p>`; }
}

/* ===== Public Shell (label diperbaiki + table-wrap + donat kecil horizontal) ===== */
function publicShell(){
  return `
  <div class="hero" style="margin-top:-8px">
    <img id="pv-hero" src="/img/gambar1.png" alt="Hero"/>
  </div>
...
function renderPublicHistory(list){
  ...
  data.forEach(it=>{ const d=new Date(it.ts||Date.now()); const tgl=d.toLocaleString('id-ID',{dateStyle:'medium',timeStyle:'short'}); const jenis=(it.type||'koreksi').toLowerCase();
    const tr=document.createElement('tr'); tr.innerHTML=`<td>${tgl}</td><td>${jenis[0].toUpperCase()+jenis.slice(1)}</td><td>${fmt(it.amount||0)}</td><td>${it.note||'-'}</td>`; body.appendChild(tr); });
}
function drawPublicPie(nama,share){ const you=Math.round(share); const other=Math.max(0,100-you); lastPvParts=[{label:nama.split(' ')[0],value:you},{label:'Lainnya',value:other}]; drawDonut(el('pvPie'),lastPvParts,PALETTE_PV); renderLegend(el('pvLegend'),lastPvParts,PALETTE_PV); el('pvShareText').textContent=`${nama} ${you}% vs Lainnya ${other}%`; }
function updatePublicStats(nas,totalAll){
  ...
  const share = totalAll>0 ? (Number(nas.saldo||0)/totalAll)*100 : 0; drawPublicPie(nas.nama, share);
  const val=num(el('pv-amount').value); if(!val){ el('pv-fee').textContent=''; return; }
  const over=Math.max(0, Math.min(val, Number(nas.saldo||0)) - free); const fee=over>0?adminFee(over):0;
  el('pv-fee').textContent = (over<=0) ? 'Tarik nominal ini: GRATIS (semua telah ≥ 30 hari).' : `Perkiraan biaya admin: ${fmt(fee)} untuk bagian dana yang < 30 hari.`;
}
async function loadPublicById(id){
  const root=el('public-root'); root.innerHTML=publicShell();
  try{
    const nas=await apiPublicById(id); ensureLots(nas); if(typeof nas.dividen!=='number') nas.dividen=0;
    if(el('pv-hero')) el('pv-hero').onerror=()=>{ el('pv-hero').style.display='none'; };
    el('pv-title').textContent=`Halo, ${nas.nama}`;
    const all=await apiGet(); const totalAll=(all.nasabah||[]).reduce((s,n)=>s+Number(n.saldo||0),0);
    renderPublicHistory(nas.history||[]); updatePublicStats(nas,totalAll);
    el('pv-amount').addEventListener('input', ()=>{ el('pv-amount').value=fmtNum(num(el('pv-amount').value)); updatePublicStats(nas,totalAll); });
    const wa=(type,amount)=>{ const nominal=num(amount); const free=freeAmount(nas); const over=Math.max(0, nominal - free); const fee=over>0?adminFee(over):0;
      const action=type==='add'?'Tambah Tabungan':'Tarik Tabungan';
      const msg=`Halo Mas Hepi, saya *${nas.nama}* (ID: ${nas.id}) ingin *${action}* sebesar *${fmt(nominal)}*.`+(type==='withdraw'&&fee>0?` Perkiraan biaya admin: *${fmt(fee)}*.`:'')+` (Link: ${originURL}/?id=${encodeURIComponent(nas.id)})`;
      return `https://wa.me/6285346861655?text=${encodeURIComponent(msg)}`; };
    el('pv-add').addEventListener('click', ()=>{ const n=num(el('pv-amount').value); if(!n){ alert('Isi nominal dulu'); return; } location.href=wa('add',n); });
    el('pv-withdraw').addEventListener('click', ()=>{ const n=num(el('pv-amount').value); if(!n){ alert('Isi nominal dulu'); return; } if(n>Number(nas.saldo||0)){ alert('Nominal melebihi saldo pokok.'); return; } location.href=wa('withdraw',n); });
  }catch(e){ root.innerHTML=`<p class="muted">Tautan tidak valid: ${e.message||e}</p>`; }
}
...
el('btnLogin')?.addEventListener('click', async ()=>{
  const u=(el('user')?.value||'').trim(); const p=(el('pass')?.value||'').trim(); const msg=el('loginMsg');
  ...
  try{ const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});
    const t=await r.text(); let j; try{j=JSON.parse(t);}catch{j={ok:false,message:t}}; if(!r.ok||!j.ok){ if(msg) msg.textContent=j.message||`Login gagal (${r.status})`; return; }
