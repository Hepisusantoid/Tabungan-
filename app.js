...
async function callPublicById(id){
...
async function callPublicByName(name){
...
/* --------- IDs & lots (FIFO) --------- */
const genId = (exist=[])=>{
  const cs='ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // no 0/O/1/I
  let s='';
  do { s=[...Array(6)].map(()=>cs[Math.floor(Math.random()*cs.length)]).join(''); }
  ...
  if (nas.lots.length === 0) {
    ...
    for (const h of sorted){
      ...
      if (h.type==='tambah' && amt>0){
        ...
        let need=amt;
        for (const l of lots){ if(need<=0) break; const take=Math.min(l.remaining, need); l.remaining-=take; need-=take; }
...
function lotsConsume(lots, amount){
  let need=amount;
  for (const l of lots){ if(need<=0) break; const t=Math.min(l.remaining, need); l.remaining-=t; need-=t; }
...
/* --------- Canvas Pie (robust) --------- */
let lastAdminParts = null;
let lastPvParts = null;

function drawPie(canvas, parts, colors){
  if (!canvas) return;

  // Pastikan sudah memiliki lebar (mobile kadang 0 px saat baru muncul)
  const rect = canvas.getBoundingClientRect();
  const baseW = Math.floor(rect.width || canvas.offsetWidth || 320);
  if (baseW === 0) {
    setTimeout(() => drawPie(canvas, parts, colors), 120);
    return;
  }

  const dpr = Math.max(1, window.devicePixelRatio || 1);
  const baseH = Math.max(220, Math.floor(baseW * 0.6)); // rasio 2:1
  canvas.width  = baseW * dpr;
  canvas.height = baseH * dpr;

  const ctx = canvas.getContext('2d');
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0); // retina scale
...
function renderAdmin(){
  ...
  if (els.statNasabah) els.statNasabah.textContent = list.length;
  if (els.statSaldo)   els.statSaldo.textContent   = fmt(total);
  if (els.statRata)    els.statRata.textContent    = fmt(list.length ? Math.round(total/list.length) : 0);
  ...
  if (els.tBody){
    ...
    els.tBody.querySelectorAll('button[data-copy]').forEach(b=>{
      b.addEventListener('click', async ()=>{
        ...
        try{ await navigator.clipboard.writeText(url); b.textContent='Disalin!'; setTimeout(()=>b.textContent='Salin',1200); }
...
/* --------- Load admin data (with migration) --------- */
async function loadData(){
  try{
    ...
    if (els.msg) els.msg.textContent = 'GET error → ' + e.message;
  }
}

/* --------- Add nasabah --------- */
if (els.btnTambah) els.btnTambah.addEventListener('click', async ()=>{
...
if (els.btnEdit) els.btnEdit.addEventListener('click', async ()=>{
...
function renderHistoryAdmin(id){
  ...
  sorted.forEach((it,row)=>{
    ...
    const cls = jenis==='tambah'?'add': jenis==='tarik'?'withdraw': jenis==='bonus'?'add':'koreksi';
    const tr = document.createElement('tr');
  ...
  els.riwTableBody.querySelectorAll('button[data-del]').forEach(b=>{
    b.addEventListener('click', async ()=>{
      ...
      if(!confirm('Hapus catatan ini? (Saldo/lots/bonus tidak berubah)')) return;
...
if (els.riwRefresh) els.riwRefresh.addEventListener('click', ()=> renderHistoryAdmin(els.riwNama.value));
if (els.riwNama) els.riwNama.addEventListener('change', ()=> renderHistoryAdmin(els.riwNama.value));

/* --------- Rename nasabah --------- */
if (els.btnRename) els.btnRename.addEventListener('click', async ()=>{
...
/* --------- Revenue share → bonus --------- */
if (els.revDistribute) els.revDistribute.addEventListener('click', async ()=>{
  ...
  try{
    ...
    els.revAmount.value='';
    renderAdmin();
    alert('Penghasilan berhasil dibagi ke seluruh nasabah (masuk ke Saldo Bonus).');
  }catch(e){ alert(e.message); }
});
...
function drawPublicPie(selfShare){
  ...
  lastPvParts = [
    { label:'Anda',    value: Math.max(0,selfShare) },
    { label:'Lainnya', value: Math.max(0,other) }
  ];
  drawPie(els.pvPie, lastPvParts, ['#46d07d','#2b3e33']);
  if (els.pvShareText) els.pvShareText.textContent = `Kepemilikan Anda sekitar ${Math.round(selfShare)}% dari total saldo seluruh nasabah.`;
}

function updatePublicStats(nas, totalAll){
  const free = freeAmount(nas);
  const locked = lockedAmount(nas);
  if (els.pvSaldo)  els.pvSaldo.textContent  = fmt(nas.saldo||0);
  if (els.pvBonus)  els.pvBonus.textContent  = fmt(nas.bonus||0);
  if (els.pvFree)   els.pvFree.textContent   = fmt(free);
  if (els.pvLocked) els.pvLocked.textContent = fmt(locked);
...
async function loadPublicById(id){
  try{
    ...
    if (els.pvTitle) els.pvTitle.textContent = `Halo, ${nas.nama}`;
    ...
    if (els.pvAmount) els.pvAmount.addEventListener('input', ()=> updatePublicStats(nas, totalAll));
    if (els.pvAdd) els.pvAdd.addEventListener('click', ()=>{
      const n = parseNum(els.pvAmount.value); if(!n){ alert('Isi nominal dulu'); return; }
      location.href = wa('add', n);
    });
    if (els.pvWithdraw) els.pvWithdraw.addEventListener('click', ()=>{
      const n = parseNum(els.pvAmount.value); if(!n){ alert('Isi nominal dulu'); return; }
...
if (els.btnLogin) els.btnLogin.addEventListener('click', async ()=>{
  ...
  try{
    ...
    setLogged(true);
  }catch{ els.loginMsg.textContent = 'Error login'; }
});
if (els.btnLogout) els.btnLogout.addEventListener('click', ()=> setLogged(false));
